...
`docs/examples/` holds the reference paperwork from Wesco that the demo's
generated documents are styled against:

- `GENERIC_MAINFREIGHT_CONOTE_EXAMPLE.pdf` — Mainfreight consignment-note
  layout (carrier is correct, customer is unrelated).
- `I-266584.jpg`, `INVOICE_EXAMPLE.pdf` — real Wesco tax-invoice layout.
- `PICKING_SLIP_EXAMPLE.jpg` — real Wesco "Factory/Picking List".

`docs/examples/RENDER_*` are screenshots of the corresponding documents
generated by this app, for quick visual diffing.
